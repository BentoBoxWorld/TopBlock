...
class AOneBlockHookTest {
    ...
    private AOneBlockHook hook;

    @BeforeEach
    void setUp() {
        when(aob.getBlockListener()).thenReturn(bl);
        when(aob.getOneBlockManager()).thenReturn(obm);
        hook = new AOneBlockHook(aob);
    }

    @Test
    void testGetGameMode() {
        assertEquals(aob, hook.getGameMode());
    }
